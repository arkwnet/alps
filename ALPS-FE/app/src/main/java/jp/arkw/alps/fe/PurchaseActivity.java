...
public class PurchaseActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button_payment_money) {
                        ...
                        (dialog, which) -> {
                            ...
                            if (isInteger(text) == true) {
                                ...
                                if (cash >= 0 && cash < 99999 && total <= cash) {
                                    ...
                                    finishPurchase(getString(R.string.payment_money), cash, change, true);
                                } else {
                                    showAlert("入力値が不正です");
                                }
                            }
            ...
            finishPurchase(getString(R.string.payment_credit), total, 0, false);
        } else if (v.getId() == R.id.button_payment_quicpay) {
            finishPurchase(getString(R.string.payment_quicpay), total, 0, false);
        } else if (v.getId() == R.id.button_payment_id) {
            finishPurchase(getString(R.string.payment_id), total, 0, false);
    ...
    public void finishPurchase(String type, int cash, int change, boolean isMoney) {
        Intent intent = new Intent(PurchaseActivity.this, MainActivity.class);
        intent.putExtra("PAYMENT", type);
        intent.putExtra("CASH", cash);
        intent.putExtra("CHANGE", change);
        intent.putExtra("IS_MONEY", isMoney);
